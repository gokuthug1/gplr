/**
 * GokuPlr v2.3.4 (Improved, acutally version 2.3.6)
 ...
 * Improvements: Sharp SVGs, CORS handling, Accessibility (ARIA), and Performance Defaults.
 */
...
(function() {
    ...
    class CustomVideoPlayer {
        ...
        constructor(videoElement) {
            ...
            // Vital for Ambient Mode and Audio Booster to work with external video sources
        ...
        #injectStyles() {
            ...
            style.textContent = `
                ...
                .tooltip-time { user-select: none; font-weight: 500; }
                ...
                .big-play-button { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); background: none; border: none; display: flex; justify-content: center; align-items: center; cursor: pointer; transition: transform 0.1s, opacity 0.2s; opacity: 1; z-index: 1; padding: 20px; border-radius: 50%; background: rgba(0,0,0,0.3); }
                ...
                .big-play-button svg { width: 64px; height: 64px; fill: var(--text-color); transition: fill var(--transition-speed); }
                ...
                .menu-back-btn { background: none; border: none; color: #eee; cursor: pointer; padding: 4px; border-radius: 4px; display: flex; align-items: center; justify-content: center; }
                ...
                .caption-settings-grid input[type="color"] { width: 30px; height: 30px; border: 1px solid #555; border-radius: 50%; padding: 0; background: none; cursor: pointer; justify-self: start; overflow: hidden; }
                .caption-settings-grid select { width: 100%; background: #333; color: white; border: 1px solid #555; border-radius: 4px; padding: 6px; font-size: 13px; }
                .caption-settings-grid .slider-container { display: flex; align-items: center; gap: 8px; width: 100%; }
                
                /* --- Shared Slider Styles --- */
                input[type=range].goku-slider { -webkit-appearance: none; appearance: none; width: 100%; height: 4px; background: var(--progress-bar-bg); border-radius: 5px; outline: none; cursor: pointer; }
                input[type=range].goku-slider::-webkit-slider-thumb { -webkit-appearance: none; appearance: none; width: 14px; height: 14px; background: var(--text-color); border-radius: 50%; cursor: pointer; margin-top: -5px; transition: transform var(--transition-speed); }
                input[type=range].goku-slider::-moz-range-thumb { width: 14px; height: 14px; background: var(--text-color); border-radius: 50%; cursor: pointer; border: none; transition: transform var(--transition-speed); }
                input[type=range].goku-slider:hover::-webkit-slider-thumb { transform: scale(1.2); background: var(--primary-color); }
                input[type=range].goku-slider:hover::-moz-range-thumb { transform: scale(1.2); background: var(--primary-color); }
                ...
                .touch-device .control-button:hover { background: none; }
                
                /* --- Responsive --- */
                @media (max-width: 600px) { .volume-slider, .touch-device .volume-slider { width: 50px; } .time-display { font-size: 12px; } .control-button svg { width: 22px; height: 22px; } .controls-bottom, .controls-left, .controls-right { gap: 4px; } .video-player-container.controls-visible.captions-on video::cue { bottom: 70px; } }
            `;
        ...
        #buildPlayerHtml() {
            ...
            // SVGs have 'shape-rendering' applied via CSS and consistent viewBox 0 0 24 24
            const playerHtml = `
                ...
                <button class="big-play-button" aria-label="Play"><svg viewBox="0 0 24 24"><path d="M8 5v14l11-7z"></path></svg></button>
                <div class="video-controls">
                    <div class="progress-bar-container" role="slider" aria-label="Seek slider" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0">
                    ...
                    <div class="controls-bottom">
                        <div class="controls-left">
                            ...
                            <div class="volume-container">
                                ...
                                <div class="volume-slider" role="slider" aria-label="Volume" aria-valuemin="0" aria-valuemax="1"><div class="volume-filled"></div><div class="volume-thumb"></div></div>
                            </div>
                            <div class="time-display"><span class="current-time">00:00</span> / <span class="total-time">00:00</span></div>
                        </div>
                        <div class="controls-right">
                            <button class="control-button volume-booster-btn" aria-label="Volume Booster"><svg viewBox="0 0 24 24"><path d="M7 2v11h3v9l7-12h-4l4-8z"></path></svg></button>
                            <button class="control-button airplay-btn" aria-label="AirPlay"><svg viewBox="0 0 24 24"><path d="M6 22h12l-6-6zM21 3H3c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h4v-2H3V5h18v12h-4v2h4c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2z"></path></svg></button>
                            <button class="control-button cast-btn" aria-label="Chromecast"><svg viewBox="0 0 24 24"><path d="M21 3H3c-1.1 0-2 .9-2 2v3h2V5h18v14h-7v2h7c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zM1 18v3h3c0-1.66-1.34-3-3-3zm0-4v2c2.76 0 5 2.24 5 5h2c0-3.87-3.13-7-7-7zm0-4v2c4.97 0 9 4.03 9 9h2c0-6.08-4.93-11-11-11z"></path></svg></button>
                                ...
                                <div class="menu-content">
                                    <div class="menu-panels-wrapper">
                                        ...
                                        <div class="menu-panel speed-panel">
                                            <div class="menu-header"><button class="menu-back-btn" data-target-panel="main" aria-label="Back"><svg viewBox="0 0 24 24"><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"></path></svg></button><span>Playback Speed</span></div>
                                            <div class="menu-panel-content"><div class="speed-slider-container"><input type="range" class="goku-slider speed-slider" min="0" max="7" value="2" step="1" aria-label="Speed"><span class="speed-panel-display">1.0×</span></div></div>
                                        </div>
                                        <!-- Panel 2: Captions Track -->
                                        <div class="menu-panel captions-track-panel">
                                            <div class="menu-header"><button class="menu-back-btn" data-target-panel="main" aria-label="Back"><svg viewBox="0 0 24 24"><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"></path></svg></button><span>Captions</span></div>
                                        ...
                                        <div class="menu-panel captions-style-panel">
                                            <div class="menu-header"><button class="menu-back-btn" data-target-panel="captions-track" aria-label="Back"><svg viewBox="0 0 24 24"><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"></path></svg></button><span>Caption Options</span></div>
                                        ...
                                        <div class="menu-panel quality-panel">
                                            <div class="menu-header"><button class="menu-back-btn" data-target-panel="main" aria-label="Back"><svg viewBox="0 0 24 24"><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"></path></svg></button><span>Quality</span></div>
                                            <div class="menu-panel-content"><div class="quality-menu-list menu-list"></div></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <button class="control-button captions-btn" aria-label="Captions"><svg viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zM4 12h4v2H4v-2zm10 6H4v-2h10v2zm6 0h-4v-2h4v2zm0-4H10v-2h10v2z"></path></svg></button>
                            ...
                            <button class="control-button fullscreen-btn" aria-label="Fullscreen"><svg class="enter-fs-icon" viewBox="0 0 24 24"><path d="M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z"></path></svg><svg class="exit-fs-icon" viewBox="0 0 24 24"><path d="M5 16h3v3h2v-5H5v2zm3-8H5v2h5V5H8v3zm6 11h2v-3h3v-2h-5v5zm2-11V5h-2v5h5V8h-3z"></path></svg></button>
                            <button class="control-button download-btn disabled" aria-label="Download"><svg viewBox="0 0 24 24"><path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"></path></svg></button>
                        </div>
                    </div>
                </div>`;
        ...
        #initializePlayerState() {
            ...
            // UX Fix: Default to 1.0x if not set (originally 0.75 in v2.3.3)
        ...
        #attachGlobalListeners() {
            ...
            // Check all prefixed versions of the fullscreenchange event
        ...
        #updateVolumeUI() {
            ...
            this.#volumeSlider.setAttribute('aria-valuenow', level);
        }
        ...
        #updateProgressBar() { 
            ...
            this.#progressBarContainer.setAttribute('aria-valuenow', Math.round(percent));
        }
        ...
        #handleScrubbing(e) {
            ...
            this.#progressBarContainer.setAttribute('aria-valuenow', Math.round(percent * 100));
        }
        #updateSeekTooltip(e) {
        ...
        async #initializeAudioBooster() {
            ...
            try {
                ...
                console.error("Audio Booster initialization failed (CORS or API):", e);
                this.#volumeBoosterBtn.style.display = 'none';
                this.#container.querySelector('.volume-booster-toggle').style.display = 'none';
                return false;
            }
        ...
        #updateAmbientEffect() {
            // Ensure the canvas context exists and dimensions match
        ...
        #loadSettings() {
            try {
                ...
                // UX Fix: Ambient Mode false by default for performance
                const defaultSettings = { 'primary-color': '#ff4081', 'caption-font-family': 'Arial', 'caption-font-size': '22px', 'caption-font-color': '#ffffff', 'caption-bg-color': 'rgba(0, 0, 0, 0.75)', 'ambient-mode': false };
